...
double k = 100000; //fixar valor molla
    ...
    for (int l = 0; l < (int)N.size(); ++l) {
        ...
        for (int j = 0; j < N[l]; ++j) {
            for (int h = 0; h < (int)N.size(); ++h)
                for (int i = 0; i < N[h]; ++i) {
                    if (l == h and l == 1) {
                        //energia elastica entre particules contigues
                        if (abs(i-j) == 1 or abs(i-j) == N[1]-1) {
                            double dx = x[l][a][j]-x[h][a][i];
                            double dy = y[l][a][j]-y[h][a][i];
                            double r = sqrt(dx*dx+dy*dy);
                            
                            E+= -0.5*k*pow((dx-d_o*(dx/r)),2);
                            E+= -0.5*k*pow((dy-d_o*(dy/r)),2);                                
                        }
                    }
                    else if (j != i or h != l) {
                        //calculem energia de i sobre j- lennard jones
                        double r2 = pow(x[l][a][j]-x[h][a][i],2) + pow(y[l][a][j]-y[h][a][i],2);
                        if(r2<rmax*rmax){
                            E += A[l][h]/pow(r2,6) - B[l][h]/pow(r2,3);
                        }
                    }
                }
        }
...
int main() {
    ...
    for (int k = 0; k < Niter; ++k) {
        ...
        if (k%1000 == 0){
            double kk=kineticE();
            double pp=potentialE(1);
            cout <<"Pot: "<< pp << "      Kin: " << kk << "       Tot: " << kk+pp << endl;
            //visualitza(0);
        }
